...
from engineering_brain.core.types import SourceType
            ...
            "vulnerabilities": [
                {
                    "cve": {
                        ...
                        "descriptions": [
                            {
                                "lang": "en",
                                "value": "XSS vulnerability in Flask-CORS allows origin bypass via crafted headers",
                            },
                        ],
                        "metrics": {
                            "cvssMetricV31": [
...
class TestOWASPChecker:
    ...
    def test_search_claim_auth_domain(self):
        ...
        assert len(sources) >= 1
        assert any(
            "authentication" in s.title.lower() or "session" in s.title.lower() for s in sources
        )
...
class TestArchitecturePatternsChecker:
    ...
    def _make_checker(self):
        from engineering_brain.validation.checkers.architecture_patterns import (
            ArchitecturePatternsChecker,
        )
...
class TestMDNSlugResolution:
    ...
    def test_extract_web_keywords_limits(self):
        ...
        text = " ".join(
            list(
                __import__(
                    "engineering_brain.validation.checkers.mdn", fromlist=["_MDN_SLUGS"]
                )._MDN_SLUGS.keys()
            )
        )
